...
# 1. LA HERRAMIENTA NUEVA: Importamos el formato estándar para servicios tipo "gatillo"
...
class FKPublisher(Node):
    def __init__(self):
        ...
        # 2. EL SERVICIO (El Teléfono): Creamos un servidor esperando llamadas en la línea '/arm/reset'
        self.srv = self.create_service(Trigger, '/arm/reset', self.reset_callback)
        
        # 3. Memoria interna: Guardamos el ángulo actual en la memoria RAM del nodo
    ...
    # 4. EL CALLBACK DEL SERVICIO: Esto SOLO se ejecuta cuando alguien hace la llamada
